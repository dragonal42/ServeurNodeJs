...
// ============================================================
function connecter() {
  // Espace Socket.IO = nom du sous-dossier dans l'adresse de la page :
  //   multi.billiar.info/courseetoile/  →  io('/courseetoile')
  // Chaque sous-site a ainsi ses propres joueurs et messages, sans se
  // mélanger avec le chat de la racine ni avec les autres sous-sites.
  // (Renommez le dossier : le jeu suit tout seul, rien à modifier ici.)
  // Pas de « room » dans la query → le serveur nous met dans la
  // room commune. Le pseudo mémorisé est envoyé dès la connexion.
  const ESPACE = '/' + (location.pathname.split('/').filter(Boolean)
                          .find(s => !s.includes('.')) || '');
  socket = io(ESPACE, {
